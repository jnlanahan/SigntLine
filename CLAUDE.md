...
- `claude.ts` — the Anthropic boundary. The per-tick turn is delegated to the agent harness (`agents/harness/runner.ts`); what stays here is key resolution, tool dependency injection, and the three pre-session calls that are not agent loops (clarifications, session plan, goal evaluation)
